...
namespace esphome
{
  namespace lg040100
  {
    ...
    private:
      ...
      void send_next_request(); // Versendet die naechste Anfrage in der Queue
      ...
      std::unordered_map<std::string, CallbackInfo> request_map_; // Dictionary fuer Requests
      std::unordered_map<std::string, int> error_count_map_;      // Fehler-Tracking
      std::string current_command_ = "";                          // Das aktuelle Kommando
    };

    using LG040100Connector = WR3223Connector;
